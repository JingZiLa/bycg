...
function openOrderItems(orderId) {
    ...
    $.ajax({
        type: 'GET',//方法类型
        url: '/api/order/querOrderGoodsById/' + orderId,
        contentType: 'application/json',
        success: function (result) {
                $('#orderItemModal').modal('show');
                var itemString = '';
                for (i = 0; i < result.length; i++) {
                    itemString += result[i].goodsName + ' x ' + result[i].goodsCount + ' 商品编号 ' + result[i].goodsId + ";<br>";
                }
    ...
    swal({
        ...
        dangerMode: true,
            ...
            if (flag) {
                $.ajax({
                    type: "POST",
                    url: "/api/order/updateOrderStatus/2",
                    contentType: "application/json",
    ...
    swal({
        ...
        dangerMode: true,
            ...
            if (flag) {
                $.ajax({
                    type: "POST",
                    url: "/api/order/updateOrderStatus/3",
                    contentType: "application/json",
                    data: JSON.stringify(ids),
                    success: function (r) {
                        swal("出库成功", {
    ...
    swal({
        ...
        dangerMode: true,
            ...
            if (flag) {
                $.ajax({
                    type: "POST",
                    url: "/api/order/updateOrderStatus/-1",
                    contentType: "application/json",
                    data: JSON.stringify(ids),
                    success: function (r) {
                        swal("关闭成功", {
